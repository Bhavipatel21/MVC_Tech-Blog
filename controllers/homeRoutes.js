...
router.get('/', async (req, res) => {
  try {
    ...
    const blogData = await TechBlog.findAll({
      include: [
        {
          model: User,
          attributes: ['name'],
    ...
    const TechBlogs = blogData.map((TechBlog) => TechBlog.get({ plain: true }));
...
router.get('/techblog/:id', async (req, res) => {
  try {
    const blogData = await TechBlog.findByPk(req.params.id, {
      include: [
        {
          model: User,
          attributes: ['name'],
